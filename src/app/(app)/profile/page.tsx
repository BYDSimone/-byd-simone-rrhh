...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data: profileData } = await supabase
        .from('profiles')
        .select(
          `id, full_name, email, phone, position, bio, birthday, hire_date, avatar_url, sucursal,
           area:areas(name)`,
        )
        .eq('id', user.id)
        .single()
      ...
      if (profileData) {
        ...
        reset({ phone: profileData.phone ?? '', bio: profileData.bio ?? '' })
      }
  ...
  const onSubmit = async (values: ProfileForm) => {
    ...
    const { error } = await supabase
      .from('profiles')
      .update({ phone: values.phone || null, bio: values.bio || null })
      .eq('id', profile.id)
    ...
    if (error) {
      ...
      setProfile((prev) => prev ? { ...prev, phone: values.phone || null, bio: values.bio || null } : prev)
      reset(values)
    }
    setSaving(false)
  }
